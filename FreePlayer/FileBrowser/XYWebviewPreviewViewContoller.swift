//
//  XYWebviewPreviewViewContoller.swift
//  FreePlayer
//
...
class XYWebviewPreviewViewContoller: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        // Add share button
        let shareButton = UIBarButtonItem(barButtonSystemItem: .action, target: self, action: #selector(XYWebviewPreviewViewContoller.shareFile(_:)))
    ...
    func processForDisplay() {
        guard let file = file, let data = try? Data(contentsOf: file.fileURL as URL) else {
            return
        }
